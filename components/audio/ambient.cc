    ...
    for (const bethesda::Subrecord& sub : record.subrecords) {
      if (sub.type == kRpld) {
        ...
        for (size_t off = 0; off + kSoundEntryStride <= sub.data.size(); off += kSoundEntryStride) {
          ...
          if (sound.plugin != 0xffff && sound.local_id != 0)
            region.sounds.push_back(sound);
        }
      }
    }
...
base::String AmbientDirector::Resolve(const AmbientContext& context) const {
  ...
  if (context.interior || context.region == 0)
    return {};
  for (const bethesda::GlobalFormId& form : regions_->SoundsFor(context.region)) {
    ...
    if (audio_ && !audio_->HasAsset(path))
      continue;
    return path;
  }
  return {};
...
void AmbientDirector::Update(const AmbientContext& context) {
  ...
  if (!decision.stop_current && !decision.start_target)
    return;  // unchanged
  ...
  if (decision.start_target) {
    ...
    if (current_voice_)
      RX_INFO("audio: ambient bed -> {}", target);
  }
  current_path_ = target;
}

void AmbientDirector::Stop() {
  if (audio_ && current_voice_)
    audio_->Stop(current_voice_, kAmbientFadeOut);
  current_voice_ = 0;
  current_path_.clear();
}
