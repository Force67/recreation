...
void TestNormals() {
  ...
  for (const rx::asset::Vertex& v : lod.vertices) {
    ...
    if (std::fabs(v.normal[2] - 1.0f) > 1e-3f)
      all_facing = false;
...
void TestSubdivide() {
  ...
  for (int i = 0; i < 4; ++i)
    for (int k = 0; k < 3; ++k)
      corners[i][k] = lod.vertices[i].position[k];
  ...
  for (int i = 0; i < 4; ++i)
    for (int k = 0; k < 3; ++k)
      if (std::fabs(lod.vertices[i].position[k] - corners[i][k]) > 1e-5f)
        corners_fixed = false;
