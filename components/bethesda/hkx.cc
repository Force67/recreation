...
base::Optional<HkxFile> HkxFile::Parse(const u8* bytes, size_t size) {
  if (size < 64)
    return base::nullopt;
  if (ReadU32(bytes) != kMagic0 || ReadU32(bytes + 4) != kMagic1)
    return base::nullopt;
  ...
  if (!little_endian)
    return base::nullopt;  // big-endian consoles: not ours
  ...
  if (file_version != 8 && file_version != 9 && file_version != 11)
    return base::nullopt;
  const size_t section_stride = file_version >= 11 ? 64 : sizeof(SectionHeader);
  if (file.pointer_size_ != 4 && file.pointer_size_ != 8)
    return base::nullopt;
  if (num_sections == 0 || num_sections > 8)
    return base::nullopt;
  if (64 + static_cast<size_t>(num_sections) * section_stride > size)
    return base::nullopt;
  ...
  if (!section_valid(classnames) || !section_valid(data))
    return base::nullopt;
  ...
  for (u32 off = data->global_fixups_offset; off + 12 <= data->virtual_fixups_offset; off += 12) {
    ...
    if (from == 0xFFFFFFFFu)
      continue;
    if (dst_section != data_section_index)
      continue;
    file.pointers_[from] = to;
  }
  ...
  for (u32 off = data->virtual_fixups_offset; off + 12 <= data->exports_offset; off += 12) {
    ...
    if (obj == 0xFFFFFFFFu)
      continue;
    if (name_off >= file.classnames_.size())
      continue;
...
base::StringRef HkxFile::CString(u64 offset) const {
  ...
  if (target == kNull || target >= data_.size())
    return {};
