...
struct HkxSkeleton {
  base::String name;
  base::Vector<HkxBone> bones;
};

struct HkxShape {
  enum class Kind { kSphere, kCapsule, kBox, kConvexVertices, kList, kTransform, kUnknown };
  Kind kind = Kind::kUnknown;
  base::String class_name;          // original havok class, for diagnostics
  ...
  base::Vector<HkxShape> children;  // list / transform
...
struct HkxRagdoll {
  base::Vector<i32> bone_to_body;  // ragdoll-skeleton bone -> body index (-1 none)
  i32 skeleton = -1;               // index into HkxPhysics::skeletons
};

struct HkxPhysics {
  base::Vector<HkxSkeleton> skeletons;
  base::Vector<HkxRigidBody> bodies;
  base::Vector<HkxConstraint> constraints;
  base::Optional<HkxRagdoll> ragdoll;
};
