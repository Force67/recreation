...
void PutDiff(base::Vector<u8>& b, u32 field_id, const base::String& s) {
  PutStr(b, "DIFF");
...
struct GraphCdbBuilder {
  base::Vector<u8> strt;
  base::Map<base::String, u32> str_off;
  ...
  struct ClassDef {
    base::String name;
    base::Vector<base::Pair<base::String, u32>> fields;  // field name, type offset
  };
  base::Vector<ClassDef> classes;

  void AddClass(const base::String& name, base::Vector<base::Pair<base::String, u32>> fields) {
    classes.push_back({name, base::move(fields)});
  }

  // A component chunk (OBJT): 'OBJT' + size + [u32 className][fields...].
  base::Vector<u8> component;                    // the streamed component chunk bytes
  base::Vector<base::Pair<u32, u32>> comp_info;  // (dbID, key)

  void PutObjt(u32 db_id, u32 key, const base::String& class_name, const base::Vector<u8>& body) {
...
base::Vector<u8> IdBody(u32 object_id) {
  base::Vector<u8> b;
...
int main() {
  ...
  base::String color, normal, emissive;
  ...
  {
    const base::String mat_path = "Materials\\Architecture\\City\\Foo.mat";
    ...
    base::String gc, gn, ge;
    Check("graph resolves the architecture material", graph_db.Lookup(mat_path, &gc, &gn, &ge));
    Check("graph base color from the texture set", gc == "textures/architecture/foo_color.dds");
