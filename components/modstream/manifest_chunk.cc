...
std::optional<ManifestChunkView> DecodeManifestChunk(const u8* data, size_t size) {
  ...
  if (view.total_size == 0 || view.total_size > kMaxManifestSize)
    return std::nullopt;
  if (view.total_chunks != ManifestChunkCount(view.total_size))
    return std::nullopt;
  if (view.chunk_index >= view.total_chunks)
    return std::nullopt;
  ...
  const u32 expected = std::min<u32>(kManifestChunkPayload, view.total_size - offset);
  if (size != kHeaderSize + expected)
    return std::nullopt;
