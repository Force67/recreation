...
Scene CompileScene(const SceneDef& def, const SceneBindings& bindings) {
  Scene out;
  auto resolve_actor = [&](i32 alias) -> u64 { return bindings.actor ? bindings.actor(alias) : 0; };
  ...
  auto emit = [&](const SceneActionDef& a) {
    switch (a.kind) {
      case SceneActionDef::Kind::kDialogue: {
        ...
        if (!info)
          return;  // the speaker has no line under this topic
        SceneAction sa;
      ...
      case SceneActionDef::Kind::kPackage: {
        ...
        f32 pos[3] = {0, 0, 0};
        f32 radius = 2.5f;
        if (!bindings.package_target || !bindings.package_target(a.package, pos, &radius))
          return;
        SceneAction sa;
