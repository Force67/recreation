...
void ScenePlayer::Start(u64 scene, base::Vector<u32> phases, f32 phase_seconds,
  ...
  if (!active_.contains(scene)) return;  // a re-entrant Stop removed it
  if (!has_phases) {
...
void ScenePlayer::Tick(f32 dt, ScenePlayerSink& sink) {
  ...
  for (u64 scene : handles) {
    if (auto* it = active_.find(scene); it != nullptr) it->timer += dt;  // accumulate once per tick
