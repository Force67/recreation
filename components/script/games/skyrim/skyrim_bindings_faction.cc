...
void ReadFactions(const bethesda::RecordStore* records,
                  const bethesda::Record& npc,
                  u16 plugin,
                  base::Vector<base::Pair<u64, i32>>* out) {
  for (const bethesda::Subrecord& sub : npc.subrecords) {
    if (sub.type != FourCc('S', 'N', 'A', 'M') || sub.data.size() < 5)
      continue;
    u32 raw;
...
i32 RecordBackedSkyrimBindings::GetFactionCount(ObjectRef actor) {
  ...
  bethesda::Record npc;
  u16 plugin;
  if (!ResolveNpc(records_, ToFormId(actor), &npc, &plugin))
    return 0;
...
ObjectRef RecordBackedSkyrimBindings::GetNthFaction(i32 index) {
  if (index < 0 || static_cast<size_t>(index) >= faction_cache_.size())
    return {};
