...
class Emitter {
 ...
 private:
  const base::String& Str(StringIndex i) const { return pex_.Str(i); }
  ...
  base::String MemberSeed(const base::String& name) {
    return Runner() ? " = new Eng(\"" + name + "\")" : " = default";
  }

  base::String DeclType(const base::String& papyrus) { return DeclTypeFor(papyrus, Harness()); }
  ...
  void EmitObject(const Object& obj) {
    ...
    for (const base::String& raw : ImplicitMembers(obj)) {
      base::String type = IEquals(raw, "::State") ? "string" : "var";
    ...
    if (Harness())
      for (const base::String& field : HarnessUnresolved(obj)) {
  ...
  void EmitMember(const MemberVariable& v) {
    base::String type = DeclType(Str(v.type));
    base::String name = Sanitize(Str(v.name));
    base::String line = "private " + type + " " + name;
    if (Runner()) {
    ...
    switch (iv.type) {
      case VariableData::Type::kInteger:
        line += " = " + base::ToString(iv.int_value);
        break;
      case VariableData::Type::kFloat:
        line += " = " + FormatFloat(iv.float_value);
        break;
      case VariableData::Type::kBool:
        line += iv.bool_value ? " = true" : " = false";
        break;
      case VariableData::Type::kString:
        line += " = " + EscapeString(Str(iv.string_index));
        break;
      default:
        break;
    }
  ...
  void EmitProperty(const Object& obj, const Property& p) {
    base::String type = DeclType(Str(p.type));
    ...
    if (p.has_setter) {
      ...
      base::String value_param = p.setter.params.empty() ? "" : Str(p.setter.params[0].name);
  ...
  void EmitFunction(const Object& obj, const NamedFunction& nf, const base::String& state) {
    const Function& fn = nf.function;
    base::String name = Sanitize(Str(nf.name));
    base::String ret = DeclType(Str(fn.return_type));
  ...
  base::String Operand(const VariableData& v) {
    switch (v.type) {
      case VariableData::Type::kNone:
        return "None";
      case VariableData::Type::kIdentifier:
        return Str(v.string_index);
      case VariableData::Type::kString:
        return EscapeString(Str(v.string_index));
      case VariableData::Type::kInteger:
  ...
  void LineComment(const base::String& text) {
    size_t start = 0;
  ...
  base::String out_;
  int indent_ = 0;
  base::UnorderedMap<base::String, base::String> backing_;
  base::UnorderedMap<base::String, base::String> fn_rename_;
  base::UnorderedMap<base::String, base::String> case_names_;
  base::UnorderedMap<base::String, base::String> case_fns_;
};
