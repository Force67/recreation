...
- Navigation and movement: `navgrid`, `pathfind`, `steering_avoidance`,
  `npc_ai`, plus the carriage pair: `carriage_records` (the linked references a
  hold carriage is built from, and the journeys its horse is given) and
  `carriage_rig` (the towed-cart physics).
