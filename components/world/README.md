...
- Navigation and movement: `navgrid`, `pathfind`, `steering_avoidance`,
  `npc_ai`, plus `road_map` (the roads read off the landscape's painted dirt,
  and routing over them) and the carriage trio: `carriage_records` (the linked
  references a hold carriage is built from, and the journeys its horse is
  given), `carriage_rig` (the towed-cart physics) and `cart_wheels` (cutting the
  wheels out of a cart's art so they can turn).
