...
inline int PickActivationTarget(const float player_pos[3],
                                const float player_fwd[3],
                                const ActivationCandidate* candidates,
                                int count,
                                float max_range,
  ...
  for (int i = 0; i < count; ++i) {
    ...
    if (dist_sq > max_range_sq || dist_sq <= 0.0f)
      continue;
...
inline bool ShoveOutOfRadius(const float pusher[3],
