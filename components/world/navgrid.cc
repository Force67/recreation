...
void NavGrid::Build(const Vec3& center,
                    f32 half_extent_m,
                    f32 cell_m,
                    f32 walk_height,
                    f32 step_tolerance,
  ...
  if (cell_m <= 0.0f || half_extent_m <= 0.0f || !probe)
    return;
...
Vec3 NavGrid::Next(const Vec3& from, const Vec3& goal, int lookahead_cells) const {
  ...
  if (!WorldToCell(from, &sx, &sz) || !WorldToCell(goal, &gx, &gz))
    return goal;
...
Vec3 NavGrid::CellToWorld(int cx, int cz) const {
  ...
  f32 y = walk_height_;
  if (!Empty() && cx >= 0 && cx < width_ && cz >= 0 && cz < width_)
    y = floor_y_[Index(cx, cz)];
  return {x, y, z};
}
