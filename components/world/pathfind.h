...
inline bool FindPath(int width,
                     int height,
                     const base::Function<bool(int, int)>& blocked,
                     PathNode start,
                     PathNode goal,
                     base::Vector<PathNode>* out_path,
                     int max_visited = 0) {
  out_path->clear();
  auto in_range = [&](int x, int y) { return x >= 0 && x < width && y >= 0 && y < height; };
  if (!in_range(start.x, start.y) || !in_range(goal.x, goal.y))
    return false;
  ...
  while (!open.empty()) {
    ...
    if (closed[ci])
      continue;
    closed[ci] = true;
    if (max_visited > 0 && ++visited > max_visited)
      return false;

    if (cur.x == goal.x && cur.y == goal.y) {
      for (int i = ci; i != -1; i = parent[i])
        out_path->push_back({i % width, i / width});
    ...
    for (int dy = -1; dy <= 1; ++dy) {
      for (int dx = -1; dx <= 1; ++dx) {
        ...
        if (!in_range(nx, ny) || blocked(nx, ny))
          continue;
