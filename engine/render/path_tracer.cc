...
// Matches PathGbufferPush in pathtrace_gbuffer.cs.hlsl (256 bytes).
...
bool PathTracer::Initialize(Device& device, VkDescriptorSetLayout bindless_layout) {
  ...
  // Reference: output (0), accum (1), tlas (8), sky cube (9). The scene
  // resources sit at 8/9 to share pathtrace_common.hlsli with the gbuffer pass.
  VkDescriptorSetLayoutBinding ref_bindings[] = {StorageImage(0), StorageImage(1),
                                                 AccelStructure(8), CombinedSampler(9)};
  set_layout_ = CreateSetLayout(dev, ref_bindings, 4);
  VkDescriptorSetLayout ref_sets[2] = {set_layout_, bindless_layout};
  layout_ = CreatePipelineLayout(dev, ref_sets, 2, sizeof(PathPush));
  if (set_layout_ == VK_NULL_HANDLE || layout_ == VK_NULL_HANDLE) return false;
  if (!CreatePipeline(dev, layout_, k_pathtrace_cs_hlsl, sizeof(k_pathtrace_cs_hlsl), &pipeline_)) {
    REC_ERROR("path tracer pipeline creation failed");
    return false;
  }

  // Denoised gbuffer: 6 NRD-input storage images (0..5), tlas (8), sky (9).
  VkDescriptorSetLayoutBinding gbuf_bindings[] = {
      StorageImage(0), StorageImage(1), StorageImage(2), StorageImage(3),
      StorageImage(4), StorageImage(5), AccelStructure(8), CombinedSampler(9)};
