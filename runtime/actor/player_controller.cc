...
bool PlayerController::Assemble() {
  ...
  // The tuning lives in player_tuning.h, shared with the server-side
  // remote-player driver, so a remote body moves exactly like the local one.
  const character::CharacterMovementSettings move =
      player_tuning::BuildMovementSettings(*ctx_.records);

  RX_INFO("player: MOVT types -> walk {:.3f} run {:.3f} sprint {:.3f} sneak {:.3f} m/s, jump {:.3f} "
          "m",
          move.walk_speed, move.run_speed, move.sprint_speed, move.crouch_speed, move.jump_height);

  // --- Capsule + eye geometry -------------------------------------------------
  character::CharacterShape shape = player_tuning::BuildCharacterShape();
