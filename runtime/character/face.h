#ifndef RECREATION_RUNTIME_CHARACTER_FACE_H_
#define RECREATION_RUNTIME_CHARACTER_FACE_H_
...
class FaceBuilder {
 ...
 private:
  ...
  const asset::Mesh* BasePartMesh(const base::String& model_path);
  ...
  struct PartTextures {
    base::String diffuse;
    base::String normal;
    base::String specular;
  };
  const PartTextures* Textures(const base::String& model_path);
  ...
  const Decoded* DecodedTexture(const base::String& path);
...
class FaceState {
 public:
  ...
  void SetMorph(const base::String& chargen_morph, f32 weight);
  void SetRaceBlend(const base::String& race_morph);  // e.g. "OrcRace"; "" disables
  ...
  base::Vector<base::String> ChargenMorphNames() const;
 ...
 private:
  ...
  struct Part {
    ...
    base::String model;                // HDPT nif path, for the part's own texture lookup
    base::String label;
  };
  ...
  struct TintLayer {
    base::String mask;         // race TINT mask path (grayscale coverage)
  ...
  base::String face_diffuse_;
  base::String face_normal_;
  ...
  base::String npc_tag_;
  base::String hair_model_;
...
#endif  // RECREATION_RUNTIME_CHARACTER_FACE_H_
