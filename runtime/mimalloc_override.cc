...
// file is compiled into each executable (see recreation_enable_mimalloc).
//
...
// no-op there. Vendored from rx, whose copy moved into its own memory tracker
// with the per-category pools work (recreation keeps RX_MIMALLOC off).
#if defined(RECREATION_MIMALLOC) && defined(_WIN32)
