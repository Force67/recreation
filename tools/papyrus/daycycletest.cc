...
using rec::ComputeSkyLighting;
using rec::SkyLighting;
using rec::WorldClock;
...
int main() {
  ...
  auto approx = [](rec::f32 a, rec::f32 b) { return std::fabs(a - b) < 1e-3f; };
  ...
  const rec::u64 hour_glob = 0x38, days_glob = 0x39, ts_glob = 0x3A;  // Skyrim time GLOBs
  bindings.set_time_globals(hour_glob, days_glob, ts_glob);
  check("GetCurrentGameTime mirrors game_days",
        approx(bindings.GetCurrentGameTime(), static_cast<rec::f32>(clock2.game_days())));
  ...
  check("GameDaysPassed global reads clock",
        approx(bindings.GetGlobalValue(ObjectRef{days_glob}), static_cast<rec::f32>(9.0 / 24.0)));
